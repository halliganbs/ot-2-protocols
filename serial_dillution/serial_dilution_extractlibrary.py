...
def run(protocol_context):
    ...
    #     plate_type, '3')
    plate = [protocol_context.load_labware('perkinelmer_384_wellplate_145ul', 
        slot) for slot in ['8','9','5','6']]
    ...
    for p in plate:
        ...
        if 'multi' in pipette_type:
            ...
            # Distribute diluent across the plate to the the number of samples
            # And add diluent to one column after the number of samples for a blank
            for col in p.columns()[1:1+num_of_dilutions]:
                pipette.distribute(
